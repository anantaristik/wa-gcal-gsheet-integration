...
async function getContentPlanningData(sheetName) {
    ...
    try {
        const response = await sheets.spreadsheets.values.get({
            spreadsheetId,
            range: `${sheetName}!A1:K`, // Hanya gunakan nama sheet
        });
...
async function getUpcomingDeadlines(sheetName) {
    ...
    const upcomingDeadlines = rows.slice(1) // Mengabaikan header
        .map(row => {
            const code = row[0];
            const format = row[1];
            const deadline = moment(row[2], 'dddd, MMMM D, YYYY'); // Parsing tanggal deadline
            const type = row[3];
            const title = row[4];
            const status = row[9];
...
async function getPostDetailByCode(sheetName, code) {
    ...
    return {
        code: row[0],
        format: row[1],
        deadline: moment(row[2], 'dddd, MMMM D, YYYY'), // Parsing tanggal
        type: row[3],
        title: row[4],
        copy: row[5],
        reference: row[6],
